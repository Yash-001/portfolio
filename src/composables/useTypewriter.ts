...
export function useTypewriter(
  ...
  function buildTimeline() {
    ...
    words.forEach((word) => {
      tl!
        .to(el.value!, { duration: word.length * typingSpeed, text: { value: word, delimiter: '' }, ease: 'none' })
        .to({}, { duration: pauseMs / 1000 })
        .to(el.value!, { duration: word.length * deletingSpeed, text: { value: '', delimiter: '' }, ease: 'none' })
        .to({}, { duration: 0.3 })
    })
  }
